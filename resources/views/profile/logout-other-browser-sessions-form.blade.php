                    ...
                    <div class="flex items-center">
                        <div>
                            @if ($session->agent->isDesktop())
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="white" class="w-8 h-8 text-gray-500">
                        ...
                        <div class="ml-3">
                            <div class="text-sm text-gray-600">
                                <p>{{ $session->agent->platform() ? $session->agent->platform() : __('Unknown') }} - {{ $session->agent->browser() ? $session->agent->browser() : __('Unknown') }}
</p> </div>
                                ...
                                <div class="text-xs text-gray-500">
                                   <p> {{ $session->ip_address }}</p>
                                    ...
                                    @if ($session->is_current_device)
                                       <p> <span class="text-green-500 font-semibold">{{ __('This device') }}</span></p>
                                    @else
                                      <p>  {{ __('Last active') }} {{ $session->last_active }}</p>
                                    @endif
                                </div>
                            </div>
                        </div>
